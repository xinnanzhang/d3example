...
//按照顺序  填充拓扑图
var OrderTopo = React.createClass({
  getInitialState(){
    return{
      centerX:0,
      centerY:0
  ...
  addOtherTag(otherAttr,oSvg){
    var _this = this;
    let centerX = oSvg.attr("width")/2;
    let centerY = oSvg.attr("height")/2;
    //线  --- 圆  ---- 中心点

    var oG = oSvg.selectAll("g").data(otherAttr).enter().append("g").attr("style","cursor:pointer");
    //线 + 方块 + 文字
    oG.append("line").attr("stroke","#ccc")
    .attr("x1",function(d,i){
      return d.x;
    })
    .attr("y1",function(d,i){
    ...
    oG.append("rect").attr("fill","#999").attr("width",20).attr("height",20).attr("rx",5)
    .attr("x",function(d,i){
      return (d.x + centerX)/2 -10;
    })
    .attr("y",function(d,i){
    ...
    oG.append("text").attr("fill","white").attr("font-size",20).attr("height",20).attr("text-anchor","middle")
    .attr("x",function(d,i){
      return (d.x + centerX)/2;
    })
    .attr("y",function(d,i){
    ...
    .on("mouseover",function(d,i){
      _this.startMove( this , 30 , 40 );
      //找到对应的线  parentNode返回当前的兄弟节点
      this.parentNode.children[0].setAttribute('stroke','blue');
      this.parentNode.children[1].setAttribute('fill','red');
    })
    .on("mouseout",function(d,i){
      _this.startMove( this, 40 , 30 );
      this.parentNode.children[0].setAttribute('stroke','#ccc');
      this.parentNode.children[1].setAttribute('fill','#999');
    });
  ...
  componentDidMount(){
    var _this = this;
    var eB1 = d3.select(".OrderTopo").append("svg").attr("width",500).attr("height",500);
    //中心点
    var centerX = eB1.attr("width")/2;
    var centerY = eB1.attr("height")/2;
    var circleNum = 9;//圆弧9等分
